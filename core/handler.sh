...
readonly NGINX_CONFIG_DIR="${PROJECT_ROOT}/nginx/conf" # Nginx 配置目录 (目标路径)
# 定义项目内子脚本的路径
...
# 定义外部配置文件和脚本的路径
readonly XRAY_CONFIG_PATH="${SCRIPT_CONFIG_DIR}/xray/config.json"    # Xray 最终配置文件路径
...
readonly XRAY_BIN_DIR="${SCRIPT_CONFIG_DIR}/bin"          # Xray 二进制目录 (项目本地)
readonly XRAY_BIN_PATH="${XRAY_BIN_DIR}/xray"             # Xray 二进制路径
readonly XSP_XRAY_SERVICE_PATH="/etc/systemd/system/xsp-xray.service"  # XSP Xray 服务文件
readonly XSP_NGINX_SERVICE_PATH="/etc/systemd/system/xsp-nginx.service" # XSP Nginx 服务文件
...
function test_and_reload_nginx() {
    ensure_nginx_support_files || return 1
    nginx -t || return 1
    if systemctl -q is-active xsp-nginx 2>/dev/null; then
        systemctl -q reload xsp-nginx 2>/dev/null || true
    else
        systemctl -q start xsp-nginx 2>/dev/null || true
    fi
}
...
function handler_update_ocsp_config() {
    ...
    *) ca_server='zerossl' ;;
    ...
    if [[ "${need_reload}" == 'y' ]] && cmd_exists 'nginx' && systemctl -q is-active xsp-nginx 2>/dev/null; then
        nginx -t && systemctl -q reload xsp-nginx 2>/dev/null || _error "nginx.conf check failed after OCSP toggle"
    fi
}
...
# 功能描述: 处理并更新 Xray 核心配置文件 (project-local xray config.json)。
#           1. 打印配置更新提示。
#           2. 从脚本配置中读取各项参数。
#           3. 加载对应配置标签的模板文件。
...
# =============================================================================
# 函数名称: cleanup_legacy_services
# 功能描述: 清理旧版本遗留的 systemd 服务、Docker 容器和 Cron 任务。
#           仅清理确认由本脚本创建的资源，不会误伤系统其他服务。
#           安全策略:
#           - xray.service: 仅在文件包含 xray 关键词时删除
#           - nginx.service: 仅在文件内容指向本项目路径时删除
#           - Docker 容器: 按精确名称匹配
#           - Cron 条目: 按脚本路径精确匹配
# 参数: 无
# 返回值: 无
# =============================================================================
function cleanup_legacy_services() {
    local cleaned=0

    # 1. 清理旧 xray 服务 (由官方 Xray-install 脚本创建)
    if [[ -f /etc/systemd/system/xray.service ]]; then
        if grep -q 'xray' /etc/systemd/system/xray.service 2>/dev/null; then
            systemctl stop xray 2>/dev/null || true
            systemctl disable xray 2>/dev/null || true
            rm -f /etc/systemd/system/xray.service
            cleaned=1
            echo -e "${GREEN}[清理]${NC} 已移除遗留服务: xray.service" >&2
        fi
    fi

    # 2. 清理旧 nginx 服务 (仅当 service 文件指向本项目路径时才清理)
    if [[ -f /etc/systemd/system/nginx.service ]]; then
        if grep -q "${PROJECT_ROOT}" /etc/systemd/system/nginx.service 2>/dev/null; then
            systemctl stop nginx 2>/dev/null || true
            systemctl disable nginx 2>/dev/null || true
            rm -f /etc/systemd/system/nginx.service
            cleaned=1
            echo -e "${GREEN}[清理]${NC} 已移除遗留服务: nginx.service (项目创建)" >&2
        fi
    fi

    # 3. 清理旧 Docker 容器 (精确名称匹配)
    if command -v docker &>/dev/null; then
        for old_container in xray-script-warp cloudreve_v3 cloudreve_v4; do
            if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^${old_container}\$"; then
                docker stop "${old_container}" 2>/dev/null || true
                docker rm "${old_container}" 2>/dev/null || true
                cleaned=1
                echo -e "${GREEN}[清理]${NC} 已移除遗留容器: ${old_container}" >&2
            fi
        done
    fi

    # 4. 清理旧 cron 条目 (按脚本路径精确匹配)
    if command -v crontab &>/dev/null; then
        local old_cron
        old_cron="$(crontab -l 2>/dev/null | grep -E 'geodata\.sh|nginx\.sh.*--update' || true)"
        if [[ -n "${old_cron}" ]]; then
            crontab -l 2>/dev/null | grep -vE 'geodata\.sh|nginx\.sh.*--update' | crontab - 2>/dev/null || true
            cleaned=1
            echo -e "${GREEN}[清理]${NC} 已移除遗留 Cron 条目" >&2
        fi
    fi

    # 执行 daemon-reload 使 systemd 重新加载配置
    if [[ ${cleaned} -eq 1 ]] && command -v systemctl &>/dev/null; then
        systemctl daemon-reload 2>/dev/null || true
    fi
}

# =============================================================================
# 函数名称: systemctl_config_xray
# 功能描述: 创建 Xray 的 systemd 服务文件 (xsp-xray.service)。
#           服务指向项目本地的 Xray 二进制文件。
# 参数: 无
# 返回值: 无
# =============================================================================
function systemctl_config_xray() {
    cat >"${XSP_XRAY_SERVICE_PATH}" <<EOF
[Unit]
Description=XSP - Xray Service
After=network.target nss-lookup.target

[Service]
Type=simple
User=root
WorkingDirectory=${SCRIPT_CONFIG_DIR}
...
# 功能描述: 安装 Xray 核心 (本地下载方式)。
#           1. 确定要安装的 Xray 版本。
#           2. 自动检测系统架构。
#           3. 从 GitHub Releases 下载对应版本的 Xray 二进制包。
#           4. 解压到项目本地 bin 目录。
#           5. 创建 systemd 服务文件。
#           6. 清理旧版本遗留服务。
# 参数:
...
function handler_install() {
    ...
    local version="${CONFIG_DATA['version']}"
    [[ -z "${version}" || "${version}" == "null" ]] && _error "Xray version not specified"

    # 如果本地二进制已存在且不是强制安装，则跳过
    if [[ -x "${XRAY_BIN_PATH}" ]] && [[ "${force_install}" == "n" ]]; then
        local current_version
        current_version="$("${XRAY_BIN_PATH}" version 2>/dev/null | head -1 | awk '{print $2}')" || true
        if [[ "${current_version}" == "${version#v}" ]]; then
            echo -e "${GREEN}[信息]${NC} Xray ${version} 已安装，跳过下载" >&2
    ...
    *)       _error "不支持的架构: ${arch}" ;;
    ...
    # 构造下载 URL
    local download_url="https://github.com/XTLS/Xray-core/releases/download/${version}/Xray-linux-${xray_arch}.zip"

    echo -e "${GREEN}[信息]${NC} 正在下载 Xray ${version} (${xray_arch})..." >&2

    # 创建临时目录
    local tmp_dir
    tmp_dir="$(mktemp -d -p "${SCRIPT_CONFIG_DIR}" -t xray.XXXXXXXX 2>/dev/null || mktemp -d -t xray.XXXXXXXX)"

    # 下载并解压
    if ! curl -fSL --progress-bar -o "${tmp_dir}/xray.zip" "${download_url}"; then
        rm -rf "${tmp_dir}"
        _error "Xray ${version} 下载失败: ${download_url}"
    fi
    ...
    if command -v unzip &>/dev/null; then
        ...
        _error "需要 unzip 命令来解压 Xray 包"
    fi

    # 移动二进制文件
    if [[ -f "${tmp_dir}/extracted/xray" ]]; then
        mv -f "${tmp_dir}/extracted/xray" "${XRAY_BIN_PATH}"
        chmod +x "${XRAY_BIN_PATH}"
        # 复制 geo 数据文件到 bin 目录 (如果包含)
        for geo_file in geoip.dat geosite.dat; do
            if [[ -f "${tmp_dir}/extracted/${geo_file}" ]]; then
                cp -f "${tmp_dir}/extracted/${geo_file}" "${XRAY_BIN_DIR}/${geo_file}"
            fi
        done
    else
        rm -rf "${tmp_dir}"
        _error "解压后的文件中未找到 xray 二进制"
    fi

    # 清理临时目录
    rm -rf "${tmp_dir}"

    echo -e "${GREEN}[信息]${NC} Xray ${version} 已安装到: ${XRAY_BIN_PATH}" >&2
...
function handler_purge() {
    # 停止并禁用 xsp-xray 服务
    systemctl stop xsp-xray 2>/dev/null || true
    systemctl disable xsp-xray 2>/dev/null || true

    # 删除 xsp-xray.service 文件
    if [[ -f "${XSP_XRAY_SERVICE_PATH}" ]]; then
        rm -f "${XSP_XRAY_SERVICE_PATH}"
    fi
...
function handler_start() {
    # 检查服务文件是否存在，避免在服务未安装时报错
    if [[ ! -f "${XSP_XRAY_SERVICE_PATH}" ]]; then
        echo -e "${YELLOW}[$(echo "$I18N_DATA" | jq -r '.title.warn')]${NC} xsp-xray 服务尚未安装" >&2
        return 0
    fi
    # 检查 Xray 服务是否活跃，如果不活跃则启动
    systemctl -q is-active xsp-xray 2>/dev/null || systemctl -q start xsp-xray 2>/dev/null || true
    # 检查 Xray 服务是否已启用，如果未启用则启用
    systemctl -q is-enabled xsp-xray 2>/dev/null || systemctl -q enable xsp-xray 2>/dev/null || true
}
...
function handler_stop() {
    ...
    systemctl -q is-active xsp-xray 2>/dev/null && systemctl -q stop xsp-xray 2>/dev/null || true
    # 检查 Xray 服务是否已启用，如果启用则禁用
    systemctl -q is-enabled xsp-xray 2>/dev/null && systemctl -q disable xsp-xray 2>/dev/null || true
}
...
function handler_restart() {
    ...
    systemctl -q is-active xsp-xray 2>/dev/null && systemctl -q restart xsp-xray 2>/dev/null || systemctl -q start xsp-xray 2>/dev/null || true
    # 检查 Xray 服务是否已启用，如果未启用则启用
    systemctl -q is-enabled xsp-xray 2>/dev/null || systemctl -q enable xsp-xray 2>/dev/null || true
}
...
function handler_geodata_cron() {
    ...
    if ! [[ -z "${XRAY_STATUS}" ]]; then
        # 如果是快速模式
        if (("${IS_QUICK}" == 0)) && crontab -l 2>/dev/null | grep -q '# xsp-geodata'; then
            # 移除现有的 GeoData Cron 任务（按 xsp 标记精确匹配）
            crontab -l 2>/dev/null | grep -v '# xsp-geodata' | crontab -
            # 打印关闭 Cron 任务的提示
            ...
            # 添加新的 GeoData Cron 任务 (每天 6:30 执行，带 xsp 标记)
            (
                crontab -l 2>/dev/null
                echo "30 6 * * * ${GEODATA_PATH} >/dev/null 2>&1 # xsp-geodata"
            ) | awk '!x[$0]++' | crontab -
            # 打印开启 Cron 任务的提示
...
function handler_nginx_cron() {
    ...
    if [[ -n "${NGINX_STATUS}" ]]; then
        # 检查是否存在 Nginx 更新的 Cron 任务（按 xsp 标记精确匹配）
        if crontab -l 2>/dev/null | grep -q '# xsp-nginx'; then
            # 移除现有的 Nginx Cron 任务
            crontab -l 2>/dev/null | grep -v '# xsp-nginx' | crontab -
            # 打印关闭 Cron 任务的提示
            echo -e "${GREEN}[$(echo "$I18N_DATA" | jq -r '.title.tip')] ${NC}$(echo "$I18N_DATA" | jq -r ".${CUR_FILE}.nginx.close_cron")" >&2
        else
            # 设置 nginx.sh 脚本为可执行
            chmod a+x "${NGINX_PATH}"
            # 添加新的 Nginx 更新 Cron 任务 (每天 3:00 执行，带 xsp 标记)
            (
                crontab -l 2>/dev/null
                echo "0 3 * * * ${NGINX_PATH} --update --brotli >/dev/null 2>&1 # xsp-nginx"
            ) | awk '!x[$0]++' | crontab -
            # 打印开启 Cron 任务的提示
...
function handler_nginx_start() {
    ensure_nginx_support_files || return 1
    # 检查服务文件是否存在
    if [[ ! -f "${XSP_NGINX_SERVICE_PATH}" ]]; then
        echo -e "${YELLOW}[$(echo "$I18N_DATA" | jq -r '.title.warn')]${NC} xsp-nginx 服务尚未安装" >&2
        return 0
    fi
    # 检查 nginx 服务是否活跃，如果不活跃则启动
    systemctl -q is-active xsp-nginx 2>/dev/null || systemctl -q start xsp-nginx 2>/dev/null || true
    # 检查 nginx 服务是否已启用，如果未启用则启用
    systemctl -q is-enabled xsp-nginx 2>/dev/null || systemctl -q enable xsp-nginx 2>/dev/null || true
}
...
function handler_nginx_stop() {
    ...
    systemctl -q is-active xsp-nginx 2>/dev/null && systemctl -q stop xsp-nginx 2>/dev/null || true
    # 检查 nginx 服务是否已启用，如果启用则禁用
    systemctl -q is-enabled xsp-nginx 2>/dev/null && systemctl -q disable xsp-nginx 2>/dev/null || true
}
...
function handler_nginx_restart() {
    ...
    systemctl -q is-active xsp-nginx 2>/dev/null && systemctl -q restart xsp-nginx 2>/dev/null || systemctl -q start xsp-nginx 2>/dev/null || true
    # 检查 nginx 服务是否已启用，如果未启用则启用
    systemctl -q is-enabled xsp-nginx 2>/dev/null || systemctl -q enable xsp-nginx 2>/dev/null || true
}
...
function main() {
    ...
    --systemd-config) systemctl_config_xray ;; # 创建 xsp-xray systemd 服务文件
    --cleanup-legacy) cleanup_legacy_services ;; # 清理遗留服务
    esac
}
