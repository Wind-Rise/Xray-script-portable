...
# --- 全局变量声明 ---
# 声明用于存储是否启用 Brotli 模块选项、是否启用 systemd 服务、语言参数和国际化数据的全局变量
...
function source_install() {
    ...
    mkdir -p "${NGINX_LOG_PATH}"                           # 创建日志目录
    mkdir -p "${PROJECT_ROOT}/nginx/bin"
    mkdir -p "${PROJECT_ROOT}/nginx/run"
    ln -sf "${NGINX_PATH}/sbin/nginx" "${PROJECT_ROOT}/nginx/bin/nginx" # 创建项目内 nginx 可执行文件
}
...
function purge_nginx() {
    ...
    rm -rf "${PROJECT_ROOT}/nginx/bin/nginx" # 删除项目内 nginx 可执行文件
...
function main() {
    ...
    while [[ $# -gt 0 ]]; do
        ...
        # 处理可选 systemd 服务选项
        --service)
            ENABLE_SYSTEMD='Y' # 设置启用 systemd 服务配置
            ;;
        # 处理帮助选项
        --help)
            show_help # 显示帮助信息并退出
